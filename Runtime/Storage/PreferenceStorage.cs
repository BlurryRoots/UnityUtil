...
namespace BlurryRoots.Storage {
	...
	public class PreferenceStorage {
		...
		public PreferenceStorageEntry[] GetAllEntries () {
			var count = this.entries.Values.Count;
			var entryArr = new PreferenceStorageEntry[count];
			var i = 0;

			foreach (var val in this.entries.Values) {
				entryArr[i++] = new PreferenceStorageEntry (val);
			}
		...
		public bool GetBool (string key, bool defaultValue = Defaults.Bool) {
			...
			var type = PreferenceStorageEntry.ValueType.Bool;
		...
		public int GetInt (string key, int defaultValue = Defaults.Int) {
			var val = defaultValue;
			var type = PreferenceStorageEntry.ValueType.Int;
		...
		public float GetFloat (string key, float defaultValue = Defaults.Float) {
			var val = defaultValue;
			var type = PreferenceStorageEntry.ValueType.Float;
		...
		public string GetString (string key, string defaultValue = Defaults.String) {
			var val = defaultValue;
			var type = PreferenceStorageEntry.ValueType.String;
		...
		public bool SetBool (string key, bool val) {
			var entry = new PreferenceStorageEntry (key) {
				Bool = val,
				Type = PreferenceStorageEntry.ValueType.Bool
			};
		...
		public float SetFloat (string key, float val) {
			var entry = new PreferenceStorageEntry (key) {
				Float = val,
				Type = PreferenceStorageEntry.ValueType.Float
			};
		...
		public int SetInt (string key, int val) {
			var entry = new PreferenceStorageEntry (key) {
				Int = val,
				Type = PreferenceStorageEntry.ValueType.Int
			};
		...
		public string SetString (string key, string val) {
			var entry = new PreferenceStorageEntry (key) {
				String = val,
				Type = PreferenceStorageEntry.ValueType.String
			};
		...
		public PreferenceStorage () {
			this.entries = new Dictionary<string, PreferenceStorageEntry> ();
		}
		...
		private void SetOrCreate (string key, PreferenceStorageEntry entry) {
			if (!this.entries.ContainsKey (key)) {
		...
		/// Storage for entries.
		/// </summary>
		private IDictionary<string, PreferenceStorageEntry> entries;
	...
	public class PreferenceStorageEntry {
		...
		public PreferenceStorageEntry (string key) {
			this.Key = key;

			this.Bool = PreferenceStorage.Defaults.Bool;
			this.Int = PreferenceStorage.Defaults.Int;
			this.Float = PreferenceStorage.Defaults.Float;
			this.String = PreferenceStorage.Defaults.String;
		...
		public PreferenceStorageEntry (PreferenceStorageEntry old) {
			this.Key = old.Key;
