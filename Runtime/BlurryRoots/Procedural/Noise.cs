...
namespace BlurryRoots {
	namespace Procedural {
		...
		public class SimplexNoiseGenerator {
			...
			public SimplexNoiseGenerator (double pScale, byte pOctaves) {
				XScale = pScale;
				YScale = pScale;
				Octaves = pOctaves;
			}

			public SimplexNoiseGenerator (double pXScale, double pYScale, byte pOctaves) {
				XScale = pXScale;
				YScale = pYScale;
				Octaves = pOctaves;
			}
			...
			public float GetNoise (double x, double y, double z) {
				if (Octaves > 1)
					return SimplexNoise.GetOctaveNoise (x * XScale, y * YScale, z * ZScale, Octaves);
				else
					return SimplexNoise.GetNoise (x * XScale, y * YScale, z * ZScale);
			}
		}
